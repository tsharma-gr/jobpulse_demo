...
pyinstaller --name "JobPulseEngine" --workpath "build_final" --distpath "dist_final" --onedir --noconsole --clean --noconfirm --add-data "backend;backend" --add-data "out;out" --add-data ".env;." --collect-all uvicorn --collect-all fastapi --collect-all pydantic --collect-all playwright --collect-all sse_starlette --collect-all supabase --collect-all pydantic_settings --collect-all dotenv --collect-all duckduckgo_search --collect-all playwright_stealth --collect-all crawl4ai --collect-all sqlalchemy --collect-all databases JobPulseEngine.py

echo 3. Setting up Playwright Chromium Browsers...
mkdir "dist_final\JobPulseEngine\browsers"
REM Temporarily set the PLAYWRIGHT_BROWSERS_PATH to the dist folder so it downloads them directly into the packaged app!
set PLAYWRIGHT_BROWSERS_PATH=%CD%\dist_final\JobPulseEngine\browsers
python -m playwright install chromium

echo ==============================================
echo BUILD COMPLETE!
echo The complete standalone app is located in the "dist_final\JobPulseEngine" folder.
